...
class FinancialViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun clearAllData(context: android.content.Context, onComplete: () -> Unit) {
        clearAllDataWithSeed(context, onComplete)
    }

    fun clearAllDataWithSeed(context: android.content.Context, onComplete: () -> Unit) {
    ...
    fun clearAllDataWithoutSeed(context: android.content.Context, onComplete: () -> Unit) {
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            context.getSharedPreferences("financial_prefs", android.content.Context.MODE_PRIVATE).edit().clear().apply()
            db.clearAllTables()
            kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.Main) {
    ...
    fun clearNamesOnly(context: android.content.Context, onComplete: () -> Unit) {
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            context.getSharedPreferences("financial_prefs", android.content.Context.MODE_PRIVATE)
                .edit()
                .remove("username")
